...
export function registerSubscriptionProviderRuntimes(
  ...
  try {
    ...
    if (unresolved.length > 0) {
      api.logger.warn(
        `[tlon-hosting] Subscription provider runtime unavailable: ${unresolved.join(', ')}`
      );
    }
    if (registered.size > 0) {
      api.logger.info(
        `[tlon-hosting] Subscription provider runtime enabled: ${[...registered].join(', ')}`
      );
    }
    return [...registered];
  } catch (error) {
    api.logger.warn(
      `[tlon-hosting] Failed to enable subscription provider runtimes: ${String(error)}`
    );
    return [];
  }
}
